...
\begin{algorithm}
  ...
  \Input{\BlankLine
    $\gamma$ : Empty directed graph with $numdots$ nodes
    \BlankLine
    $\epsilon$ : A randomly ordered list of all possible edges between nodes, with each edge of form (Parent, Child)}
  \BlankLine
  \Begin{
    \For{$edge \in \epsilon$}{
...
\begin{algorithm}
  ...
  \Begin{
    \For{$ n \in \n_{1:num\_dots}$}{
      ...
      $Velocity_x \leftarrow \MVNORMAL(mean\_velocity_x , covmat_x)$\;
      $Velocity_y \leftarrow \MVNORMAL(mean\_velocity_y , covmat_y)$\;
...
\begin{algorithm}
\DontPrintSemicolon
\KwData{$G=(X,U)$ such that $G^{tc}$ is an order.} \KwResult{$G’=(X,V)$ with $V\subseteq U$ such that $G’^{tc}$ is an interval order.}
\Begin{
  $V \longleftarrow U$\;
  ...
  \For{$x \in X$}{
    \If{$NbPredInMin(x) = 0$ {and} $NbPredNotInMin(x) = 0$}{
      $AppendToMin(x)$}
    }
...
\For{$ y \in S-ImSucc(x)$}{
\{ remove from $V$ all the arcs $zy$ : \}\; \For{$z \in ImPred(y) \cap Min$}{
remove the arc $zy$ from $V$\;
$NbSuccInS(z) \longleftarrow NbSuccInS(z) - 1$\;
move $z$ in $T$ to the list preceding its present list\; \{i.e. If $z \in T[k]$, move $z$ from $T[k]$ to
$T[k-1]$\}\; }
